...
public class GetAllAnnos {
    ...
    @Test
    @PostConstruct
    public void addAllConfigs() {
        // 包下面的类
        Set<Class<?>> clazzs = getClasses("com.lsh.demo.bootstone.web");
        if (clazzs == null) {
            return;
        }

        System.out.print(clazzs.size() + "");
        // 某类或者接口的子类
        Set<Class<?>> inInterface = getByInterface(Object.class, clazzs);
        System.out.print(inInterface.size() + "");
        ...
        for (Class<?> clazz : clazzs) {
            ...
            for (Annotation anno : annos) {
                System.out.println("类注解" + clazz.getSimpleName().concat(".").concat(anno.annotationType().getSimpleName()));
            }
            ...
            for (Method method : methods) {
                ...
                for (Annotation annotation : annotations) {
                    System.out.print("方法注解" + clazz.getSimpleName().concat(".").concat(method.getName()).concat(".")
                            .concat(annotation.annotationType().getSimpleName()));

                    //过滤ratelimitanno,缓存信息-redis
                    if (annotation instanceof RateLimitAnno) {
