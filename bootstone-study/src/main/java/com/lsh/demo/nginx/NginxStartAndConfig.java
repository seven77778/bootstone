...
public class  NginxStartAndConfig {
     ...
     *
     补充下 对很多高并发的机器来说, CPU, 内存, 带宽都不是瓶颈, 瓶颈是网络IO次数. 用比较通俗的话来比喻就是, 快递一次给你送十个包裹, 你完全无压力, 但是如果他一天分十次给你送十个包裹, 要十次interrupt你, 你就崩溃了. 反向代理起的作用就是当你的收发室, 收集了十个包裹再一次发给你.

     但是如果这样利用反向代理服务器, 反向代理服务器就必须部署在另外一台物理机器上, 否则就是你自己兼职做自己的收发室了, 毫无意义.

     这种做法很奢侈, 适合于财大气粗的土豪公司.

     如果可以的话, 还是换一个多队列网卡硬件, 或者用软件虚拟的RPS. 我曾经用过RPS, 效果确实很明显, 软中断很均匀的分布在机器的多个CPU核上, 吞吐能力提升显著.
     */
